...
export default class PostFull extends Component {
  ...
  componentDidMount = () => {
    ...
    axios.get("http://localhost:7003/v1/comments/" + this.props.match.params.threadId).then(res => {
    ...
    axios.get(`http://localhost:7003/v1/posts/getPostById/${this.props.match.params.threadId}`).then(res => {
  ...
  postReply = () => {
    ...
    axios.post("http://localhost:7003/v1/comments/", { content: this.state.text, postId: this.props.match.params.threadId, username: "taaah" }).then(res => {
      console.log(res.data);
  ...
  likeComment = (id) => {
    console.log(id, "myid")
    axios.get("http://localhost:7003/v1/comments/162").then(res => {
      console.log(res.data);
    }).catch((err) => {
      console.log(err)
    })
    ...
    const replyList = this.state.replies.map(reply => {
      return (
        ...
        <ul key={reply.id} style={{ paddingTop: '30px', listStyle: "none", paddingLeft: "0px" }}>
          <Card className='mx-auto responsive ' >
            <Card.Body>
    ...
    return (
      <Loader loaded={this.state.loaded}>
        <Jumbotron className="container-Jumbotron list">
          <Container style={{backgroundColor:"#ECEBEB"}}>
            <Card className='mx-auto responsive' border="Secondary" >
              <Card.Body>
                <div className="row justify-content-end">
                  <div >
                ...
                <div>
                  <svg onClick={() => this.setState({ replycard: true })} className="bi bi-reply-fill" width="3em" height="3em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
