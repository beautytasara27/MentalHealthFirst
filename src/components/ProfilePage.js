import React from "react";
import {AuthConsumer} from './Context/AuthContext'
import {Button, Row, Form } from 'react-bootstrap'
import firebase from '../components/config/fbConfig'

class ProfilePage extends React.Component{
state= {
  file: {},
  imgUrl : '',
  displayName: '',
}
handleChange = (e) => {
    this.setState({
      [e.target.id]: e.target.value
    })
  }


  chooseFile = (e) => {
    this.setState({file : e.target.files[0]});
      
  }
  changeDisplayName = () =>{
    var user = firebase.auth().currentUser;
    user.updateProfile({
      displayName: this.state.displayName
    }).catch((err)=>{
      console.log("update displayname",err)
    })
  }

  handleSubmit = (e) =>{
    e.preventDefault();
    var user = firebase.auth().currentUser;
    firebase.storage().ref('users/' + user.uid + '/profile.jpg').put(this.state.file).then((snapshot)=>{
      firebase.storage().ref('users/' + user.uid + '/profile.jpg').getDownloadURL().then(imgUrl =>{
          this.setState({imgUrl: imgUrl});
          user.updateProfile({
            //displayName: this.state.displayName,
            photoURL: imgUrl
        }).catch((err)=>{
          console.log("on update profile pic: ", err)
        })
      }).catch((err)=>{
        console.log("downloading url", err)
      })

    }).catch((err)=>{
      console.log("on storingerror :", err)
    })
    console.log(" oh my oh my", user)

  }


  render(){
  return (
    
    <AuthConsumer>
    {({logout, isAuth}) =>(
    <div  style={{backgroundColor: '#ffd1dc',position: "center", padding:'200px'}}>
      <div style={{backgroundColor: 'lightblue',padding:'20px',position: "center",}} className="flex border flex-col items-center md:flex-row md:items-start border-blue-400 px-3 py-4">
        <div>
        <div>
        <h3 >{isAuth? isAuth.diplayName:"username"}</h3>
        <Form >
        <input id="displayName" type="text" onChange={this.handleChange}></input>
        <Button onClick={this.changeDisplayName}>Change</Button>
        </Form>
        </div>
          <img src={isAuth?isAuth.photoURL : "data:image/jpeg;base64,"} alt="profile pic" width="200" height="200"
          style={{
            backgroundSize: "cover",
            height: "200px",
            width: "200px"
          }}  className="border border-blue-300"/>
         
         <div>
         <Form >
         <input type="file" onChange={this.chooseFile}></input>
         <Button onClick={this.handleSubmit}>Upload</Button>
         </Form>
         </div>
         
        </div>
        <div className = "md:pl-4">
          
          <Row>
            <h3 className = "italic">{isAuth? isAuth.email : "email"}</h3>
            <svg class="bi bi-pen" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" d="M5.707 13.707a1 1 0 0 1-.39.242l-3 1a1 1 0 0 1-1.266-1.265l1-3a1 1 0 0 1 .242-.391L10.086 2.5a2 2 0 0 1 2.828 0l.586.586a2 2 0 0 1 0 2.828l-7.793 7.793zM3 11l7.793-7.793a1 1 0 0 1 1.414 0l.586.586a1 1 0 0 1 0 1.414L5 13l-3 1 1-3z"/>
              <path fill-rule="evenodd" d="M9.854 2.56a.5.5 0 0 0-.708 0L5.854 5.855a.5.5 0 0 1-.708-.708L8.44 1.854a1.5 1.5 0 0 1 2.122 0l.293.292a.5.5 0 0 1-.707.708l-.293-.293z"/>
              <path d="M13.293 1.207a1 1 0 0 1 1.414 0l.03.03a1 1 0 0 1 .03 1.383L13.5 4 12 2.5l1.293-1.293z"/>
            </svg>
           
          </Row>
        </div>
      </div>
      
      <Button onClick={logout}>Sign out</Button>
      
    </div>
    )}
    </AuthConsumer>
  ) 
}
}
export default ProfilePage;